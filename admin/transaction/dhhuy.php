    ...
    <div class="col-md-12">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                <?php foreach($don as $row): ?>
    <?php 
    $id = intval($row['users_id']);
    $users = $db->fetchID("users", $id);

    // Kiểm tra nếu $users là null, nếu có thì thay thế bằng thông tin mặc định
    if (!$users) {
        $users = [
            'name' => 'Thông tin không có',
            'address' => 'Thông tin không có',
            'phone' => 'Thông tin không có',
            'email' => 'Thông tin không có',
            'update_at' => 'Thông tin không có',
        ];
    }
    ?>
    <tr>
        ...
        <td>
            <?php if ($row['done'] == 0): ?>
            <a href="status.php?id=<?php echo $row['id'] ?>" class="btn btn-xs <?php echo $row['status'] == 0 ? 'btn-danger' : 'btn-info' ?>">
                <?php echo $row['status'] == 0 ? 'Chưa xử lý' : 'Đã xử lý'; ?>
            </a>
            <?php else: ?>
            <a href="status.php?id=<?php echo $row['id'] ?>" class="btn btn-xs <?php echo $row['status'] == 0 ? 'btn-danger disabled' : 'btn-info' ?>">
                <?php echo $row['status'] == 0 ? 'Chưa xử lý' : 'Đã xử lý'; ?>
            </a>
            <?php endif; ?>
        </td>
        ...
        <td>
            <?php if ($row['huy'] == 1): ?>
            <a href="done.php?id=<?php echo $row['id'] ?>" class="btn btn-xs <?php echo $row['done'] == 0 ? 'btn-warning' : 'btn-primary' ?>">
                <?php echo $row['done'] == 0 ? 'Người dùng hủy đơn' : 'Đã xác nhận'; ?>
            </a>
            <?php endif; ?>
        </td>
        <td>
